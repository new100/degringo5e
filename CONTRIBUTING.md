# Contributing to foundryvtt/degringo5e
...
Cloning this repository and either placing it in or symlinking it to your `Data/systems/degringo5e` user data directory is all that is necessary to run this within Foundry VTT. However, if you want to make changes to either the LESS stylesheets or the compendia, there are some developer tools which will make your life easier.
...
Converts the LESS in `./less` to the final `degringo5e.css`.
...
Non-English languages are not contained within the core degringo5e system, but instead they are managed by specialized [localization modules](https://foundryvtt.com/packages/tag/translation).
...
```text
https://github.com/foundryvtt/degringo5e/releases/download/release-1.6.3/degringo5e-1.6.3.zip
                                                     └─ Tag Name ──┘     └─ V ─┘ (version)
```
