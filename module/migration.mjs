...
export async function migrateWorld({ bypassVersionCheck=false }={}) {
  ...
  for ( const [actor, valid] of actors ) {
    try {
      ...
      err.message = `Failed degringo5e system migration for Actor ${actor.name}: ${err.message}`;
      console.error(err);
    }
    incrementProgress();
  }
  ...
  for ( const [item, valid] of items ) {
    try {
      ...
      err.message = `Failed degringo5e system migration for Item ${item.name}: ${err.message}`;
      console.error(err);
    }
    incrementProgress();
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed degringo5e system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
    incrementProgress();
  }
  ...
  for ( const table of game.tables ) {
    try {
      ...
      err.message = `Failed degringo5e system migration for RollTable ${table.name}: ${err.message}`;
      console.error(err);
    }
    incrementProgress();
  }
  ...
  for ( const s of game.scenes ) {
    try {
      ...
      err.message = `Failed degringo5e system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
    ...
    for ( const token of s.tokens ) {
      ...
      try {
        ...
        err.message = `Failed degringo5e system migration for ActorDelta [${token.id}]: ${err.message}`;
        console.error(err);
      }
      incrementProgress();
    }
  ...
  game.settings.set("degringo5e", "systemMigrationVersion", game.system.version);
  ...
  try {
    ...
    for ( let doc of documents ) {
      ...
      // Handle migration failures
        ...
        err.message = `Failed degringo5e system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
        console.error(err);
        if ( strict ) throw err;
      }
...
export async function refreshCompendium(pack, { bypassVersionCheck, migrate=true }={}) {
  ...
  if ( migrate ) {
    try {
      ...
      err.message = `Failed degringo5e system migration pack ${pack.collection}: ${err.message}`;
      console.error(err);
      return;
    }
  }
...
export async function migrateArmorClass(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.DEGRINGO5E.armorTypes));
...
export async function migrateSettings() {
  ...
  const disableExperienceTracking = game.settings.storage.get("world")
    ?.find(s => s.key === "degringo5e.disableExperienceTracking")?.value;
  const levelingMode = game.settings.storage.get("world")?.find(s => s.key === "degringo5e.levelingMode")?.value;
  if ( (disableExperienceTracking !== undefined) && (levelingMode === undefined) ) {
    await game.settings.set("degringo5e", "levelingMode", "noxp");
  }
}
...
export function migrateItemData(item, itemData, migrationData, flags={}) {
  ...
  // Migrate embedded effects
  if ( itemData.effects ) {
    const riders = foundry.utils.getProperty(itemData, "flags.degringo5e.riders.effect");
    if ( riders?.length ) updateData["flags.degringo5e.riders.effect"] = riders;
    const effects = migrateEffects(itemData, migrationData, updateData, flags);
    if ( riders?.length === updateData["flags.degringo5e.riders.effect"]?.length ) {
      delete updateData["flags.degringo5e.riders.effect"];
    }
    if ( effects.length > 0 ) updateData.effects = effects;
  }
  ...
  // Migrate properties
  const migratedProperties = foundry.utils.getProperty(itemData, "flags.degringo5e.migratedProperties");
  if ( migratedProperties?.length ) {
    ...
    updateData["flags.degringo5e.-=migratedProperties"] = null;
  }

  if ( foundry.utils.getProperty(itemData, "flags.degringo5e.persistSourceMigration") ) {
    flags.persistSourceMigration = true;
    updateData["flags.degringo5e.-=persistSourceMigration"] = null;
  }
...
export function migrateEffects(parent, migrationData, itemUpdateData, flags={}) {
  ...
  return parent.effects.reduce((arr, e) => {
    ...
    if ( effectData.flags?.degringo5e?.rider ) {
      itemUpdateData["flags.degringo5e.riders.effect"] ??= [];
      itemUpdateData["flags.degringo5e.riders.effect"].push(effectData._id);
      effectUpdate["flags.degringo5e.-=rider"] = null;
    }
    if ( effectData.flags?.degringo5e?.persistSourceMigration ) {
      flags.persistSourceMigration = true;
      effectUpdate["flags.degringo5e.-=persistSourceMigration"] = null;
    }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/degringo5e/json/icon-migration.json");
    const spellIcons = await fetch("systems/degringo5e/json/spell-icon-migration.json");
...
function _migrateActorFlags(actorData, updateData) {
  const initiativeAdv = foundry.utils.getProperty(actorData, "flags.degringo5e.initiativeAdv");
  if ( initiativeAdv ) {
    const key = "system.attributes.init.roll.mode";
    updateData[key] = Math.min(1, (foundry.utils.getProperty(actorData, key) ?? 0) + 1);
    updateData["flags.degringo5e.-=initiativeAdv"] = null;
  }
  return updateData;
}
...
function _migrateTokenImage(actorData, updateData) {
  const oldSystemPNG = /^systems\/degringo5e\/tokens\/([a-z]+)\/([A-z]+).png$/;
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/degringo5e/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
...
function _migrateItemUses(item, itemData, updateData, flags) {
  const value = foundry.utils.getProperty(itemData, "flags.degringo5e.migratedUses");
  ...
  if ( value !== undefined ) updateData["flags.degringo5e.-=migratedUses"] = null;
}
...
/**
 * Migrate macros from the old 'degringo5e.rollItemMacro' and 'degringo5e.macros' commands to the new location.
 * @param {object} macro       Macro data to migrate.
 * @param {object} updateData  Existing update to expand upon.
 * @returns {object}           The updateData to apply.
 */
function _migrateMacroCommands(macro, updateData) {
  if ( macro.command.includes("game.degringo5e.rollItemMacro") ) {
    updateData.command = macro.command.replaceAll("game.degringo5e.rollItemMacro", "degringo5e.documents.macro.rollItem");
  } else if ( macro.command.includes("game.degringo5e.macros.") ) {
    updateData.command = macro.command.replaceAll("game.degringo5e.macros.", "degringo5e.documents.macro.");
  }
  return updateData;
}
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.degringo5e || null;
    return flags5e ? {degringo5e: flags5e} : {};
  };
  await pack.configure({locked: false});
