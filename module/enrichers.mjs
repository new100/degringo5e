...
async function enrichAttack(config, label, options) {
  ...
  for ( const value of config.values ) {
    if ( value in CONFIG.DEGRINGO5E.attackModes ) config.attackMode = value;
  ...
  span.innerHTML = game.i18n.format(`EDITOR.DEGRINGO5E.Inline.Attack${config._rules === "2014" ? "Long" : "Short"}`, {
    formula: createRollLink(displayFormula).outerHTML
  });

  if ( config.format === "extended" ) {
    const type = game.i18n.format(`DEGRINGO5E.ATTACK.Formatted.${config._rules}`, {
      type: game.i18n.getListFormatter({ type: "disjunction" }).format(
        Array.from(activity?.validAttackTypes ?? []).map(t => CONFIG.DEGRINGO5E.attackTypes[t]?.label)
      ),
      classification: CONFIG.DEGRINGO5E.attackClassifications[activity?.attack.type.classification]?.label ?? ""
    }).trim();
    ...
    full.innerHTML = game.i18n.format("EDITOR.DEGRINGO5E.Inline.AttackExtended", {
...
async function enrichAward(config, label, options) {
  ...
  if ( parsed.xp ) entries.push(`
    <span class="award-entry">
      ${formatNumber(parsed.xp)} ${game.i18n.localize("DEGRINGO5E.ExperiencePoints.Abbreviation")}
    </span>
  `);

  let award = game.i18n.getListFormatter({ type: "unit" }).format(entries);
  if ( parsed.each ) award = game.i18n.format("EDITOR.DEGRINGO5E.Inline.AwardEach", { award });

  block.innerHTML += `
    ${award}
    <a class="award-link" data-action="awardRequest">
      <i class="fa-solid fa-trophy"></i> ${label ?? game.i18n.localize("DEGRINGO5E.Award.Action")}
    </a>
  `;
...
async function enrichCheck(config, label, options) {
  ...
  for ( let value of config.values ) {
    ...
    if ( slug in CONFIG.DEGRINGO5E.enrichmentLookup.abilities ) config.ability = slug;
    else if ( slug in CONFIG.DEGRINGO5E.enrichmentLookup.skills ) config.skill.push(slug);
    else if ( slug in CONFIG.DEGRINGO5E.enrichmentLookup.tools ) config.tool.push(slug);
  ...
  if ( activity ) {
    ...
    for ( const associated of activity.check.associated ) {
      if ( associated in CONFIG.DEGRINGO5E.skills ) config.skill.push(associated);
      else if ( associated in CONFIG.DEGRINGO5E.tools ) config.tool.push(associated);
    }
    delete config.activity;
  }

  // TODO: Support "spellcasting" ability
  let abilityConfig = CONFIG.DEGRINGO5E.enrichmentLookup.abilities[slugify(config.ability)];
  ...
  for ( let [index, skill] of config.skill.entries() ) {
    const skillConfig = CONFIG.DEGRINGO5E.enrichmentLookup.skills[slugify(skill)];
    if ( skillConfig ) {
  ...
  for ( const tool of config.tool ) {
    const toolConfig = CONFIG.DEGRINGO5E.tools[slugify(tool)];
    const toolUUID = CONFIG.DEGRINGO5E.enrichmentLookup.tools[slugify(tool)];
  ...
  if ( complex ) {
    ...
    for ( const [ability, associated] of groups.entries() ) {
      ...
      if ( associated.length > 1 ) parts.push(
        game.i18n.format("EDITOR.DEGRINGO5E.Inline.SpecificCheck", {
          ability: CONFIG.DEGRINGO5E.enrichmentLookup.abilities[ability].label,
    ...
    if ( config.dc && !config.hideDC ) {
      label = game.i18n.format("EDITOR.DEGRINGO5E.Inline.DC", { dc: config.dc, check: label });
    }
    label = game.i18n.format(`EDITOR.DEGRINGO5E.Inline.Check${config.format === "long" ? "Long" : "Short"}`, { check: label });
...
function createCheckRequestButtons(dataset) {
  ...
  return [
    ...skills.map(skill => createRequestButton({
      ability: CONFIG.DEGRINGO5E.skills[skill].ability, ...baseDataset, format: "short", skill, type: "skill"
    })),
    ...tools.map(tool => createRequestButton({
      ability: CONFIG.DEGRINGO5E.tools[tool]?.ability, ...baseDataset, format: "short", tool, type: "tool"
    }))
  ];
}
...
async function enrichSave(config, label, options) {
  ...
  for ( let value of config.values ) {
    const slug = foundry.utils.getType(value) === "string" ? slugify(value) : value;
    if ( slug in CONFIG.DEGRINGO5E.enrichmentLookup.abilities ) config.ability.push(slug);
    else if ( Number.isNumeric(value) ) config.dc = Number(value);
    else config[value] = true;
  }
  config.ability = config.ability
    .filter(a => a in CONFIG.DEGRINGO5E.enrichmentLookup.abilities)
    .map(a => CONFIG.DEGRINGO5E.enrichmentLookup.abilities[a].key ?? a);
  ...
  if ( label ) label = createRollLink(label);
    ...
    label = game.i18n.format(`EDITOR.DEGRINGO5E.Inline.Save${config.format === "long" ? "Long" : "Short"}`, { save: label });
    const template = document.createElement("template");
    template.innerHTML = label;
    label = template;
  }
...
async function enrichDamage(configs, label, options) {
  ...
  for ( const c of configs ) {
    ...
    for ( const value of c.values ) {
      if ( value in CONFIG.DEGRINGO5E.damageTypes ) c.type.push(value);
      else if ( value in CONFIG.DEGRINGO5E.healingTypes ) c.type.push(value);
      else if ( value in CONFIG.DEGRINGO5E.attackModes ) config.attackMode = value;
  ...
  for ( const [idx, formula] of config.formulas.entries() ) {
    ...
    const types = type?.split("|")
      .map(t => CONFIG.DEGRINGO5E.damageTypes[t]?.label ?? CONFIG.DEGRINGO5E.healingTypes[t]?.label)
      .filter(_ => _);
    ...
    parts.push(game.i18n.format(`EDITOR.DEGRINGO5E.Inline.Damage${localizationType}`, localizationData));
  }
  ...
  if ( config.average && (parts.length === 2) ) {
    link.innerHTML = game.i18n.format("EDITOR.DEGRINGO5E.Inline.DamageDouble", { first: parts[0], second: parts[1] });
  } else {
  ...
  if ( config.format === "extended" ) {
    ...
    span.innerHTML = game.i18n.format("EDITOR.DEGRINGO5E.Inline.DamageExtended", { damage: link.outerHTML });
    return span;
  }
...
async function enrichReference(config, label, options) {
  let key;
  let source;
  let type = Object.keys(config).find(k => k in CONFIG.DEGRINGO5E.ruleTypes);
  if ( type ) {
    key = slugify(config[type]);
    const { references } = CONFIG.DEGRINGO5E.ruleTypes[type] ?? {};
    source = foundry.utils.getProperty(CONFIG.DEGRINGO5E, references)?.[key];
  } else if ( config.values.length ) {
    key = slugify(config.values.join(""));
    for ( const [t, { references }] of Object.entries(CONFIG.DEGRINGO5E.ruleTypes) ) {
      source = foundry.utils.getProperty(CONFIG.DEGRINGO5E, references)?.[key];
      if ( source ) {
        type = t;
        break;
  ...
  if ( (type === "condition") && (config.apply !== false) ) {
    ...
    apply.setAttribute("aria-label", game.i18n.localize("EDITOR.DEGRINGO5E.Inline.ApplyStatus"));
...
export function createRollLabel(config) {
  const { label: ability, abbreviation } = CONFIG.DEGRINGO5E.abilities[config.ability] ?? {};
  const skill = CONFIG.DEGRINGO5E.skills[config.skill]?.label;
  const toolUUID = CONFIG.DEGRINGO5E.enrichmentLookup.tools[config.tool];
  ...
  switch ( config.type ) {
    ...
    case "tool":
      if ( ability && (skill || tool) ) {
        label = game.i18n.format("EDITOR.DEGRINGO5E.Inline.SpecificCheck", { ability, type: skill ?? tool });
      } else {
        label = ability;
      }
      if ( config.passive ) {
        label = game.i18n.format(
          `EDITOR.DEGRINGO5E.Inline.${showDC ? "DC" : ""}Passive${longSuffix}`, { dc: config.dc, check: label }
        );
      } else {
        if ( showDC ) label = game.i18n.format("EDITOR.DEGRINGO5E.Inline.DC", { dc: config.dc, check: label });
        label = game.i18n.format(`EDITOR.DEGRINGO5E.Inline.Check${longSuffix}`, { check: label });
      }
      break;
    case "concentration":
    case "save":
      if ( config.type === "save" ) label = ability;
      else label = `${game.i18n.localize("DEGRINGO5E.Concentration")} ${ability ? `(${abbreviation})` : ""}`;
      if ( showDC ) label = game.i18n.format("EDITOR.DEGRINGO5E.Inline.DC", { dc: config.dc, check: label });
      label = game.i18n.format(`EDITOR.DEGRINGO5E.Inline.Save${longSuffix}`, { save: label });
      break;
    default:
      return "";
  }
...
function createRequestLink(label, dataset) {
  ...
  if ( game.user.isGM ) {
    ...
    gmLink.dataset.tooltip = "EDITOR.DEGRINGO5E.Inline.RequestRoll";
...
async function rollAction(event) {
  ...
  const options = { event };
  if ( ability in CONFIG.DEGRINGO5E.abilities ) options.ability = ability;
  ...
  if ( (action === "roll") || !game.user.isGM ) {
    ...
    try {
      ...
      if ( !actors.length ) {
        ui.notifications.warn("EDITOR.DEGRINGO5E.Inline.Warning.NoActor", { localize: true });
        return;
      }
...
async function rollAttack(event) {
  ...
  const messageConfig = {
    data: {
      ...
      flavor: game.i18n.localize("DEGRINGO5E.AttackRoll"),
...
async function rollDamage(event) {
  ...
  const messageConfig = {
    ...
    data: {
      ...
      flavor: game.i18n.localize(`DEGRINGO5E.${rollType === "healing" ? "Healing" : "Damage"}Roll`),
      speaker: ChatMessage.implementation.getSpeaker()
    }
  };
...
async function useItem({ rollActivityUuid, rollActivityName, rollItemUuid, rollItemName, rollItemActor }={}) {
  ...
  if ( item ) {
    if ( rollActivityName ) {
      ...
      else ui.notifications.warn(game.i18n.format("EDITOR.DEGRINGO5E.Inline.Warning.NoActivityOnItem", {
  ...
  else ui.notifications.warn(game.i18n.format("EDITOR.DEGRINGO5E.Inline.Warning.NoItemOnActor", {
