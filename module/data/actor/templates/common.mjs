...
export default class CommonTemplate extends ActorDataModel.mixin(CurrencyTemplate) {
    ...
    return this.mergeSchema(super.defineSchema(), {
      abilities: new MappingField(new SchemaField({
        value: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 10, label: "DEGRINGO5E.AbilityScore"
        }),
        proficient: new NumberField({
          required: true, integer: true, min: 0, max: 1, initial: 0, label: "DEGRINGO5E.ProficiencyLevel"
        }),
        max: new NumberField({
          required: true, integer: true, nullable: true, min: 0, initial: null, label: "DEGRINGO5E.AbilityScoreMax"
        }),
        bonuses: new SchemaField({
          check: new FormulaField({ required: true, label: "DEGRINGO5E.AbilityCheckBonus" }),
          save: new FormulaField({ required: true, label: "DEGRINGO5E.SaveBonus" })
        }, { label: "DEGRINGO5E.AbilityBonuses" }),
        check: new RollConfigField({ ability: false }),
        save: new RollConfigField({ ability: false })
      }), {
        initialKeys: CONFIG.DEGRINGO5E.abilities, initialValue: this._initialAbilityValue.bind(this),
        initialKeysOnly: true, label: "DEGRINGO5E.Abilities"
      })
    });
  }
  ...
  static _initialAbilityValue(key, initial, existing) {
    const config = CONFIG.DEGRINGO5E.abilities[key];
    if ( config ) {
  ...
  prepareAbilities({ rollData={}, originalSaves }={}) {
    ...
    for ( const [id, abl] of Object.entries(this.abilities) ) {
      ...
      if ( !Number.isFinite(abl.max) ) abl.max = CONFIG.DEGRINGO5E.maxAbilityScore;
