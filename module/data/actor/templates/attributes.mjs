...
export default class AttributesFields {
  ...
  static get armorClass() {
    return {
      calc: new StringField({ initial: "default", label: "DEGRINGO5E.ArmorClassCalculation" }),
      flat: new NumberField({ integer: true, min: 0, label: "DEGRINGO5E.ArmorClassFlat" }),
      formula: new FormulaField({ deterministic: true, label: "DEGRINGO5E.ArmorClassFormula" })
    };
  }
  ...
  static get common() {
    return {
      ac: new SchemaField(this.armorClass, { label: "DEGRINGO5E.ArmorClass" }),
      init: new RollConfigField({
        ability: "",
        bonus: new FormulaField({ required: true, label: "DEGRINGO5E.InitiativeBonus" })
      }, { label: "DEGRINGO5E.Initiative" }),
  ...
  static get creature() {
    return {
      attunement: new SchemaField({
        max: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 3, label: "DEGRINGO5E.AttunementMax"
        })
      }, { label: "DEGRINGO5E.Attunement" }),
      senses: new SensesField(),
      spellcasting: new StringField({ required: true, blank: true, label: "DEGRINGO5E.SpellAbility" }),
      exhaustion: new NumberField({
        required: true, nullable: false, integer: true, min: 0, initial: 0, label: "DEGRINGO5E.Exhaustion"
      }),
      concentration: new RollConfigField({
        ability: "",
        bonuses: new SchemaField({
          save: new FormulaField({ required: true, label: "DEGRINGO5E.ConcentrationBonus" })
        }),
        limit: new NumberField({ integer: true, min: 0, initial: 1, label: "DEGRINGO5E.ConcentrationLimit" })
      }, { label: "DEGRINGO5E.Concentration" }),
      loyalty: new SchemaField({
        value: new NumberField({ integer: true, min: 0, max: 20, label: "DEGRINGO5E.Loyalty" })
      })
    };
  }
  ...
  static prepareArmorClass(rollData) {
    ...
    let cfg = CONFIG.DEGRINGO5E.armorClasses[ac.calc];
    if ( !cfg ) {
      ac.calc = "flat";
      if ( Number.isNumeric(ac.value) ) ac.flat = Number(ac.value);
      cfg = CONFIG.DEGRINGO5E.armorClasses.flat;
    }

    // Identify Equipped Items
    const { armors, shields } = this.parent.itemTypes.equipment.reduce((obj, equip) => {
      if ( !equip.system.equipped || !(equip.system.type.value in CONFIG.DEGRINGO5E.armorTypes)) return obj;
      ...
      default:
        ...
        if ( armors.length ) {
          if ( armors.length > 1 ) this.parent._preparationWarnings.push({
            message: game.i18n.localize("DEGRINGO5E.WarnMultipleArmor"), type: "warning"
          });
        ...
        try {
          const replaced = replaceFormulaData(formula, rollData, {
            actor: this, missing: null, property: game.i18n.localize("DEGRINGO5E.ArmorClass")
          });
          ac.base = replaced ? new Roll(replaced).evaluateSync().total : 0;
        } catch(err) {
          this.parent._preparationWarnings.push({
            message: game.i18n.format("DEGRINGO5E.WarnBadACFormula", { formula }), link: "armor", type: "error"
          });
          const replaced = Roll.replaceFormulaData(CONFIG.DEGRINGO5E.armorClasses.default.formula, rollData);
    ...
    if ( shields.length ) {
      if ( shields.length > 1 ) this.parent._preparationWarnings.push({
        message: game.i18n.localize("DEGRINGO5E.WarnMultipleShields"), type: "warning"
      });
  ...
  static prepareConcentration(rollData) {
    ...
    const abilityId = concentration.ability || CONFIG.DEGRINGO5E.defaultAbilities.concentration;
  ...
  static prepareEncumbrance(rollData, { validateItem }={}) {
    const config = CONFIG.DEGRINGO5E.encumbrance;
    const encumbrance = this.attributes.encumbrance ??= {};
    ...
    for ( const type in CONFIG.DEGRINGO5E.movementTypes ) {
      ...
      if ( noMovement || (crawl && (type !== "walk")) ) speed = 0;
        ...
        if ( heavilyEncumbered ) {
          speed = Math.max(0, speed - (CONFIG.DEGRINGO5E.encumbrance.speedReduction.heavilyEncumbered[units] ?? 0));
        } else if ( encumbered ) {
          speed = Math.max(0, speed - (CONFIG.DEGRINGO5E.encumbrance.speedReduction.encumbered[units] ?? 0));
        }
        if ( exceedingCarryingCapacity ) {
          speed = Math.min(speed, CONFIG.DEGRINGO5E.encumbrance.speedReduction.exceedingCarryingCapacity[units] ?? 0);
        }
      }
  ...
  static prepareSpellcastingAbility() {
    ...
    this.attributes.spell.abilityLabel = CONFIG.DEGRINGO5E.abilities[this.attributes.spellcasting]?.label ?? "";
