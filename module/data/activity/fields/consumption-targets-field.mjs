...
export class ConsumptionTargetData extends foundry.abstract.DataModel {
  ...
  get combatOnly() {
    ...
    return recovery.every(r => CONFIG.DEGRINGO5E.limitedUsePeriods[r.period]?.type === "combat");
  }
   ...
   */
  get validTargets() {
    const config = CONFIG.DEGRINGO5E.activityConsumptionTypes[this.type];
  ...
  async consume(config, updates) {
    const typeConfig = CONFIG.DEGRINGO5E.activityConsumptionTypes[this.type];
  ...
  static async consumeActivityUses(config, updates) {
    const result = await this._usesConsumption(config, {
      ...
      type: game.i18n.format("DEGRINGO5E.CONSUMPTION.Type.ActivityUses.Warning", {
  ...
  static async consumeAttribute(config, updates) {
    ...
    if ( !foundry.utils.hasProperty(this.actor, keyPath) ) throw new ConsumptionError(
      game.i18n.format("DEGRINGO5E.CONSUMPTION.Warning.MissingAttribute", {
    ...
    let warningMessage;
    if ( (cost > 0) && !current ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.None";
    else if ( current < cost ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      ...
      type: game.i18n.format("DEGRINGO5E.CONSUMPTION.Type.Attribute.Warning", { attribute: this.target })
    }));
  ...
  static async consumeHitDice(config, updates) {
    ...
    let warningMessage;
    if ( !validClasses.length ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.MissingHitDice";
    else if ( (cost > 0) && !total ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.None";
    else if ( total < cost ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      ...
      throw new ConsumptionError(game.i18n.format(warningMessage, {
        ...
        type: game.i18n.format("DEGRINGO5E.CONSUMPTION.Type.HitDice.Warning", { denomination })
      }));
    }
  ...
  static async consumeItemUses(config, updates) {
    ...
    if ( !item ) throw new ConsumptionError(game.i18n.format("DEGRINGO5E.CONSUMPTION.Warning.MissingItem", {
      activity: this.activity.name, item: this.item.name
    }));

    const result = await this._usesConsumption(config, {
      uses: item.system.uses,
      type: game.i18n.format("DEGRINGO5E.CONSUMPTION.Type.ItemUses.Warning", { name: this.item.name }),
      rolls: updates.rolls,
  ...
  static async consumeMaterial(config, updates) {
    ...
    let warningMessage;
    if ( cost > 0 && !item.system.quantity ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.None";
    else if ( cost > item.system.quantity ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      available: formatNumber(item.system.quantity), cost: formatNumber(cost),
      type: game.i18n.format("DEGRINGO5E.CONSUMPTION.Type.Material.Warning", { name: item.name })
    }));
  ...
  static async consumeSpellSlots(config, updates) {
    const levelNumber = Math.clamp(
      this.resolveLevel({ config, rolls: updates.rolls }), 1, Object.keys(CONFIG.DEGRINGO5E.spellLevels).length - 1
    );
    ...
    let warningMessage;
    if ( !levelData?.max ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.MissingSpellSlot";
    else if ( (cost > 0) && !levelData.value ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.None";
    else if ( newValue < 0 ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      const level = CONFIG.DEGRINGO5E.spellLevels[levelNumber];
      const type = game.i18n.format("DEGRINGO5E.CONSUMPTION.Type.SpellSlots.Warning", { level });
      throw new ConsumptionError(game.i18n.format(warningMessage, {
  ...
  async _usesConsumption(config, { uses, type, rolls, delta }) {
    ...
    let warningMessage;
    if ( cost > 0 && !uses.value ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.None";
    else if ( cost > uses.value ) warningMessage = "DEGRINGO5E.CONSUMPTION.Warning.NotEnough";
  ...
  static consumptionLabelsActivityUses(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.ActivityUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `DEGRINGO5E.CONSUMPTION.Type.ActivityUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
          availableUse: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.Use.${usesPluralRule}`)
        }
      ),
  ...
  static consumptionLabelsAttribute(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.Attribute.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `DEGRINGO5E.CONSUMPTION.Type.Attribute.PromptHint${increaseKey}`,
  ...
  static consumptionLabelsHitDice(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    let denomination;
    if ( this.target === "smallest" ) denomination = game.i18n.localize("DEGRINGO5E.ConsumeHitDiceSmallest");
    else if ( this.target === "largest" ) denomination = game.i18n.localize("DEGRINGO5E.ConsumeHitDiceLargest");
    ...
    return {
      label: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.HitDice.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `DEGRINGO5E.CONSUMPTION.Type.HitDice.PromptHint${increaseKey}`,
        {
          cost, denomination: denomination.toLowerCase(),
          die: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.HitDie.${pluralRule}`),
  ...
  static consumptionLabelsItemUses(config, { consumed }={}) {
    ...
    const itemName = item ? item.name : game.i18n.localize("DEGRINGO5E.CONSUMPTION.Target.ThisItem").toLowerCase();
    ...
    if ( simplifiedCost > uses.value ) warn = true;
      ...
      type: "warn",
      message: game.i18n.format("DEGRINGO5E.CONSUMPTION.Warning.WillDestroy", { item: itemName })
    });

    return {
      label: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.ItemUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `DEGRINGO5E.CONSUMPTION.Type.ItemUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
  ...
  static consumptionLabelsMaterial(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.Material.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `DEGRINGO5E.CONSUMPTION.Type.Material.PromptHint${increaseKey}`,
        {
          cost,
          item: item ? `<em>${item.name}</em>` : game.i18n.localize("DEGRINGO5E.CONSUMPTION.Target.ThisItem").toLowerCase(),
  ...
  static consumptionLabelsSpellSlots(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    const levelNumber = Math.clamp(this.resolveLevel({ config }), 1, Object.keys(CONFIG.DEGRINGO5E.spellLevels).length - 1);
    const level = CONFIG.DEGRINGO5E.spellLevels[levelNumber].toLowerCase();
    const available = this.actor.system.spells?.[`spell${levelNumber}`]?.value ?? 0;
    return {
      label: game.i18n.localize(`DEGRINGO5E.CONSUMPTION.Type.SpellSlots.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `DEGRINGO5E.CONSUMPTION.Type.SpellSlots.PromptHint${increaseKey}`,
        {
          cost,
          slot: game.i18n.format(`DEGRINGO5E.CONSUMPTION.Type.SpellSlot.${pluralRule}`, { level }),
          available: formatNumber(available)
        }
      ),
      warn: simplifiedCost > available
  ...
  static validAttributeTargets() {
    ...
    return TokenDocument.implementation.getConsumedAttributes(this.actor.type).map(attr => {
      let group;
      if ( attr.startsWith("abilities.") ) group = game.i18n.localize("DEGRINGO5E.AbilityScorePl");
      else if ( attr.startsWith("currency.") ) group = game.i18n.localize("DEGRINGO5E.Currency");
      else if ( attr.startsWith("spells.") ) group = game.i18n.localize("DEGRINGO5E.CONSUMPTION.Type.SpellSlots.Label");
      else if ( attr.startsWith("attributes.movement.") ) group = game.i18n.localize("DEGRINGO5E.Speed");
      else if ( attr.startsWith("attributes.senses.") ) group = game.i18n.localize("DEGRINGO5E.Senses");
      else if ( attr.startsWith("resources.") ) group = game.i18n.localize("DEGRINGO5E.Resources");
  ...
  static validHitDiceTargets() {
    return [
      { value: "smallest", label: game.i18n.localize("DEGRINGO5E.ConsumeHitDiceSmallest") },
      ...CONFIG.DEGRINGO5E.hitDieTypes.map(d => ({ value: d, label: d })),
      { value: "largest", label: game.i18n.localize("DEGRINGO5E.ConsumeHitDiceLargest") }
    ];
  }
  ...
  static validItemUsesTargets() {
    const makeLabel = (name, item) => {
      ...
      if ( uses.max && (uses.recovery?.length === 1) && (uses.recovery[0].type === "recoverAll")
        ...
        const per = CONFIG.DEGRINGO5E.limitedUsePeriods[uses.recovery[0].period]?.abbreviation;
        label = game.i18n.format("DEGRINGO5E.AbilityUseConsumableLabel", { max: uses.max, per });
      }
      else label = game.i18n.format("DEGRINGO5E.AbilityUseChargesLabel", { value: uses.value });
      return `${name} (${label})`;
    };
    return [
      { value: "", label: makeLabel(game.i18n.localize("DEGRINGO5E.CONSUMPTION.Target.ThisItem"), this.item) },
      { rule: true },
      ...(this.actor?.items ?? [])
  ...
  static validSpellSlotsTargets() {
    return Object.entries(CONFIG.DEGRINGO5E.spellLevels).reduce((arr, [value, label]) => {
