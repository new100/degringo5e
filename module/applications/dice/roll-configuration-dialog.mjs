...
export default class RollConfigurationDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      title: "DEGRINGO5E.RollConfiguration.Title",
  ...
  static PARTS = {
    formulas: {
      template: "systems/degringo5e/templates/dice/roll-formulas.hbs"
    },
    configuration: {
      template: "systems/degringo5e/templates/dice/roll-configuration.hbs"
    },
    buttons: {
      template: "systems/degringo5e/templates/dice/roll-buttons.hbs"
    }
  };
  ...
  _identifyDiceTerms() {
    ...
    const identifyTerm = term => {
      ...
      for ( let i = 0; i < term.number; i++ ) dice.push({
        icon: `systems/degringo5e/icons/svg/dice/${term.denomination}.svg`,
  ...
  async _prepareButtonsContext(context, options) {
    context.buttons = {
      roll: {
        ...
        label: game.i18n.localize("DEGRINGO5E.Roll")
      }
    };
    return context;
  }
  ...
  async _prepareConfigurationContext(context, options) {
    context.fields = [{
      field: new foundry.data.fields.StringField({
        label: game.i18n.localize("DEGRINGO5E.RollMode"), blank: false, required: true
      }),
      name: "rollMode",
  ...
  _buildConfig(config, formData, index) {
     ...
     * on the rolling method (e.g. `degringo5e.buildSkillRollConfig`, `degringo5e.buildAbilityCheckRollConfig`,
     * `degringo5e.buildRollConfig`).
     * @function degringo5e.buildRollConfig
     * @memberof hookEvents
    ...
    for ( const hookName of this.#config.hookNames ?? [""] ) {
      Hooks.callAll(`degringo5e.build${hookName.capitalize()}RollConfig`, this, config, formData, index);
    }
     ...
     * depending on the rolling method (e.g. `degringo5e.postBuildSkillRollConfig`, `degringo5e.postBuildAbilityCheckRollConfig`,
     * `degringo5e.postBuildRollConfig`).
     * @function degringo5e.postBuildRollConfig
     * @memberof hookEvents
     ...
     */
    for ( const hookName of this.#config.hookNames ?? [""] ) {
      Hooks.callAll(`degringo5e.postBuild${hookName.capitalize()}RollConfig`, this.config, config, index, {
        app: this, formData
      });
    }
  ...
  /** @override */
  _onClose(options={}) {
    if ( !options.degringo5e?.submitted ) this.#rolls = [];
  }
